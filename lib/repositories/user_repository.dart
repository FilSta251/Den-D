...
class UserValidationException extends UserRepositoryException {
  UserValidationException(String message) : super(message, code: 'VALIDATION_ERROR');
}

class UserNotFoundException extends UserRepositoryException {
  UserNotFoundException(String userId) : super('Uživatel "$userId" nebyl nalezen', code: 'USER_NOT_FOUND');
}
...
class UserValidator {
  ...
  static void validateName(String name) {
    ...
    if (name.trim().length < minNameLength) {
      throw UserValidationException('Jméno musí mít alespoň $minNameLength znaky');
    }
    if (name.length > maxNameLength) {
      throw UserValidationException('Jméno nesmí být delší než $maxNameLength znaků');
    }
    if (!RegExp(r'^[a-žA-Ž\s]+$', unicode: true).hasMatch(name.trim())) {
      throw UserValidationException('Jméno smí obsahovat pouze písmena a mezery');
    }
  }
  ...
  static void validatePassword(String password) {
    ...
    if (!RegExp(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)').hasMatch(password)) {
      throw UserValidationException('Heslo musí obsahovat malé písmeno, velké písmeno a číslo');
    }
  }
}
...
class UserRepository {
  ...
  /// Počítadlo pokusů pro různé operace
  ...
  User? get cachedUser {
    if (_cachedUser != null && _cacheTimestamp != null) {
      final isExpired = DateTime.now().difference(_cacheTimestamp!) > _cacheTimeout;
      if (isExpired) {
        _logger.debug('Cache expired, clearing cached user', category: 'user_repository');
        _cachedUser = null;
    ...
    try {
      ...
      if (_lastFetchTime != null && !forceRefresh) {
        ...
        if (timeSinceLastFetch < _minFetchInterval) {
          _logger.debug('Rate limited, using cache', category: 'user_repository');
      ...
      if (!forceRefresh && isCacheValid && cachedUser?.id == userId) {
        _logger.debug('Returning cached user', category: 'user_repository', extra: {'userId': userId});
        return cachedUser!;
      }

      _logger.info('Fetching user profile', category: 'user_repository', extra: {
        'userId': userId,
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          final jitter = (baseDelay * 0.1 * (DateTime.now().millisecondsSinceEpoch % 1000 - 500) / 500).round();
  ...
  static Future<User> _fetchUserProfileIsolate(_FetchUserProfileParams params) async {
    try {
      ...
      throw NetworkException('Chyba při načítání z Firestore', originalError: e);
    }
  }
  ...
  Future<void> updateUserProfile(
    ...
    try {
      ...
      _logger.info('Updating user profile', category: 'user_repository', extra: {
        'userId': user.id,
        'merge': merge,
      ...
      throw NetworkException('Chyba při aktualizaci profilu uživatele', originalError: e);
    }
  }
  ...
  static Future<void> _updateUserProfileIsolate(_UpdateUserProfileParams params) async {
    try {
      ...
      if (params.merge) {
        await params.firestore.collection('users').doc(params.user.id).set(data, SetOptions(merge: true));
      } else {
        await params.firestore.collection('users').doc(params.user.id).update(data);
      }
    } catch (e) {
  ...
  Future<void> updateUsersProfilesBatch(List<User> users) async {
    ...
    try {
      ...
      _logger.info('Batch updating user profiles', category: 'user_repository', extra: {
        'userCount': users.length,
      });

      await compute(_batchUpdateIsolate, _BatchUpdateParams(users: users, firestore: _firestore));
  ...
  Stream<User> userProfileStream(String userId) {
    try {
      ...
      _logger.info('Creating user profile stream', category: 'user_repository', extra: {
        'userId': userId,
      });
      ...
      final controller = StreamController<User>.broadcast(
        onCancel: () {
          ...
          _logger.debug('User stream cancelled', category: 'user_repository', extra: {'userId': userId});
        },
      );
      ...
      _usersCollection.doc(userId).snapshots().listen(
        ...
        onError: (error) {
          _logger.error('User stream error', 
            ...
            extra: {'userId': userId});
          controller.addError(NetworkException('Chyba v real-time streamu', originalError: error));
        },
      );
  ...
  Future<User> signInWithEmail(String email, String password) async {
    ...
    try {
      ...
      if (fbUser == null) {
        throw UserRepositoryException("Přihlášení selhalo (UserCredential.user == null).");
      }

      _logger.info('Firebase auth successful', category: 'user_repository', extra: {
        'uid': fbUser.uid,
      });

      final loadedUser = await fetchUserProfile(userId: fbUser.uid, forceRefresh: true);
  ...
  Future<User> signUpWithEmail(
    ...
    try {
      ...
      final fbUser = result.user;
      if (fbUser == null) {
        throw UserRepositoryException('Registrace selhala (UserCredential.user == null).');
      }

      _logger.info('Firebase auth registration successful', category: 'user_repository', extra: {
        'uid': fbUser.uid,
      });
  ...
  static Future<void> _createUserProfileIsolate(_CreateUserProfileParams params) async {
    try {
      ...
      throw NetworkException('Chyba při vytváření profilu v Firestore', originalError: e);
    }
  }
  ...
  Future<void> saveUserLocally(User user) async {
    try {
      ...
      _logger.error('Failed to save user locally', 
        ...
        exception: e,  // OPRAVENO: změněno z 'error' na 'exception'
        extra: {'userId': user.id});
      throw UserRepositoryException('Chyba při ukládání lokálních dat', originalError: e);
    }
  }
  ...
  static Future<void> _saveUserLocallyIsolate(User user) async {
    ...
    if (user.weddingDate != null) {
      await prefs.setString('user_wedding_date', user.weddingDate!.toIso8601String());
    } else {
  ...
  Future<User?> loadUserFromLocal() async {
    try {
      _logger.debug('Loading user from local storage', category: 'user_repository');
      ...
      if (localUser != null) {
        try {
          ...
          _logger.debug('User loaded from local storage', category: 'user_repository', extra: {
            'userId': localUser.id,
          });
        } catch (e) {
      ...
      _logger.error('Failed to load user from local storage', 
        category: 'user_repository',
        exception: e);  // OPRAVENO: změněno z 'error' na 'exception'
      throw UserRepositoryException('Chyba při načítání lokálních dat', originalError: e);
    }
  }
  ...
  Future<void> clearLocalData() async {
    try {
      ...
      _logger.error('Failed to clear local data', 
        category: 'user_repository',
        exception: e);  // OPRAVENO: změněno z 'error' na 'exception'
      throw UserRepositoryException('Chyba při mazání lokálních dat', originalError: e);
    }
  }
  ...
  static Future<void> _clearLocalDataIsolate(_) async {
    ...
    final keys = prefs.getKeys().where((key) => key.startsWith('user_')).toList();
 ...
 /// Získat nebo zvýšit počet pokusů pro operaci
 ...
 void dispose() {
   ...
   for (final completer in _completerMap.values) {
     if (!completer.isCompleted) {
       completer.completeError(UserRepositoryException('Batch loader byl ukončen'));
     }
   }
   _completerMap.clear();
 }
...
extension UserRepositoryBatch on UserRepository {
 ...
 Future<List<User>> _batchFetchUsers(List<String> userIds) async {
   ...
   try {
     ...
     throw NetworkException('Chyba při batch načítání uživatelů', originalError: e);
   }
 }
}