// lib/services/local_guests_service.dart - OPRAVENÁ VERZE
...
/// Služba pro lokální správu hostů využívající základní třídu
class LocalGuestsService extends BaseLocalStorageService<Guest> 
  ...
  /// Načte stoly ze separátního klíče
  Future<void> _loadTables() async {
    try {
      ...
      if (tablesData != null && tablesData.isNotEmpty) {
        ...
        _tables = jsonData
            .map((json) => TableArrangement.fromJson(json as Map<String, dynamic>))
            .toList();
      } else {
        // Výchozí stůl "Nepřiřazen" pokud neexistují žádné stoly
        _tables = [
          TableArrangement(
            id: 'unassigned',
            name: 'Nepřiřazen',
  ...
  Future<void> addTable(TableArrangement table) async {
    // Kontrola, zda už neexistuje stůl se stejným jménem
    if (_tables.any((t) => t.name == table.name)) {
      throw Exception('Stůl s názvem ${table.name} již existuje');
    }
  ...
  Future<void> removeTable(String tableId) async {
    ...
    // Přesuneme všechny hosty z tohoto stolu na "Nepřiřazen"
    final guestsToUpdate = items.where((guest) => guest.table == tableName).toList();
  ...
  Future<void> updateTable(TableArrangement updatedTable) async {
    ...
    if (oldName != updatedTable.name) {
      final guestsToUpdate = items.where((guest) => guest.table == oldName).toList();
      for (final guest in guestsToUpdate) {
  ...
  Map<String, dynamic> getGuestStatistics() {
    return {
      'total': itemCount,
      'male': getGuestsByGender('Muž').length,
      'female': getGuestsByGender('Žena').length,
      'other': getGuestsByGender('Jiné').length,
  ...
  Map<String, Map<String, dynamic>> getTableUtilization() {
    ...
    for (final table in _tables) {
      ...
      utilization[table.name] = {
        ...
        'available': table.maxCapacity > 0 ? table.maxCapacity - guestsAtTable.length : null,
        'percentage': table.maxCapacity > 0 
            ? (guestsAtTable.length / table.maxCapacity * 100).round()
            : 0,
        'isFull': table.maxCapacity > 0 && guestsAtTable.length >= table.maxCapacity,
      };
    }
  ...
  /// Kontrola, zda je možné přiřadit hosta ke stolu
  ...
  void setGuestsAndTablesWithoutNotify(List<Guest> guests, List<TableArrangement> tables) {
    setItemsWithoutNotify(guests);
    
    // Zajistíme, že vždy existuje výchozí stůl
