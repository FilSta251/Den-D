...
class ChecklistManager extends ChangeNotifier {
  ...
  Future<void> _syncPendingChanges() async {
    ...
    try {
            ...
            case 'category':
              ...
              switch (operation) {
                ...
                case 'update':
                  ...
                  break;
                case 'remove':
                  final defaultCategoryId = change['defaultCategoryId'] as String;
                  await _cloudService.removeCategory(category.id, defaultCategoryId);
                  break;
              }
              break;
          }
      ...
      // Synchronizace celé kolekce
        ...
        await _cloudService.syncFromLocal(_localService.tasks, _localService.categories);
  ...
  Future<void> _forceInitialSync() async {
    ...
    try {
      ...
      if (cloudTasks.isNotEmpty || cloudCategories.isNotEmpty) {
        ...
        _localService.setTasksAndCategoriesWithoutNotify(cloudTasks, cloudCategories);
        
        _localService.addListener(_handleLocalChanges);
      } 
      // Pokud máme lokální data a cloud je prázdný, nahrajeme na cloud
      else if ((_localService.tasks.isNotEmpty || _localService.categories.isNotEmpty) && _isOnline) {
        await _cloudService.syncFromLocal(_localService.tasks, _localService.categories);
      }
  ...
  void _enableCloudSync() {
    ...
    _tasksCloudSubscription = _cloudService.getTasksStream().listen((cloudTasks) {
      if (cloudTasks.isNotEmpty) {
        final currentCategories = _localService.categories;
        
        _localService.removeListener(_handleLocalChanges);
        _localService.setTasksAndCategoriesWithoutNotify(cloudTasks, currentCategories);
        _localService.addListener(_handleLocalChanges);
    ...
    _categoriesCloudSubscription = _cloudService.getCategoriesStream().listen((cloudCategories) {
      if (cloudCategories.isNotEmpty) {
        final currentTasks = _localService.tasks;
        
        _localService.removeListener(_handleLocalChanges);
        _localService.setTasksAndCategoriesWithoutNotify(currentTasks, cloudCategories);
        _localService.addListener(_handleLocalChanges);
  ...
  // === Veřejné metody pro práci s úkoly ===
  ...
  Future<void> removeCategory(String categoryId) async {
    final category = _localService.categories.firstWhere((c) => c.id == categoryId);
    final defaultCategoryId = LocalChecklistService.defaultCategories.first['id'] as String;
  ...
  void clearAllData() {
    ...
    for (final category in allCategories) {
      if (!LocalChecklistService.defaultCategories.any((c) => c['id'] == category.id)) {
        _pendingChanges.add({
          'operation': 'remove',
          'type': 'category',
          'data': category,
 ...
 List<Task> getTasksByCategory(String categoryId) => _localService.getTasksByCategory(categoryId);
 ...
 List<Task> getTasksByPriority(int priority) => _localService.getTasksByPriority(priority);
 ...
 Map<String, dynamic> getChecklistStatistics() => _localService.getChecklistStatistics();
 
 /// Získá statistiky podle kategorií.
 Map<String, Map<String, dynamic>> getCategoryStatistics() => _localService.getCategoryStatistics();
