// lib/services/local_calendar_service.dart
...
class LocalCalendarService extends BaseLocalStorageService<CalendarEvent> 
  ...
  List<CalendarEvent> getEventsForDay(DateTime day) {
    ...
    return findItems((event) {
      ...
      return (event.startTime.isAfter(dayStart) || event.startTime.isAtSameMomentAs(dayStart)) &&
  ...
  List<CalendarEvent> getEventsWithReminder() {
    return findItems((event) => event.reminder != null);
  }
  ...
  Map<String, dynamic> getCalendarStatistics() {
    ...
    final withReminder = getEventsWithReminder();
  ...
  List<CalendarEvent> findTimeConflicts(DateTime startTime, DateTime? endTime) {
    return findItems((event) {
      ...
      // Kontrola překryvu
      return (startTime.isBefore(eventEnd) && checkEnd.isAfter(event.startTime));
    });
  }
  ...
  String exportToICal() {
    ...
    for (final event in events) {
      ...
      if (event.allDay) {
        buffer.writeln('DTSTART;VALUE=DATE:${_formatDateForICal(event.startTime)}');
        if (event.endTime != null) {
          buffer.writeln('DTEND;VALUE=DATE:${_formatDateForICal(event.endTime!)}');
        }
      } else {
      ...
      if (event.reminder != null) {
        ...
        switch (event.reminder!.type) {
          case 'minutes':
            trigger += '${event.reminder!.value}M';
            break;
          case 'hours':
            trigger += '${event.reminder!.value}H';
            break;
          case 'days':
            trigger += '${event.reminder!.value}D';
            break;
        }
