...
/// Služba pro cloudovou synchronizaci checklistu svatby.
/// 
...
class CloudChecklistService {
  ...
  /// Vrací referenci na kolekci úkolů pro aktuálního uživatele.
  ...
  /// Vrací referenci na kolekci kategorií pro aktuálního uživatele.
  ...
  /// Získá stream úkolů, který se aktualizuje v reálném čase.
  ...
  /// Získá stream kategorií, který se aktualizuje v reálném čase.
  ...
  Future<List<Task>> fetchTasks() async {
    ...
    try {
      ...
      debugPrint('Chyba při načítání úkolů: $e');
      return _cachedTasks ?? [];
    }
  }
  
  /// Načte kategorie z Firestore.
 ...
 Future<void> updateCategory(TaskCategory category) async {
   ...
   await _withRetry<void>(() async {
     await _getCategoriesCollection().doc(category.id).update(category.toJson());
 ...
 Future<void> removeCategory(String categoryId, String defaultCategoryId) async {
   if (_userId == null) {
     throw Exception("Uživatel není přihlášen");
   }
   
   await _withRetry<void>(() async {
     // Najdeme všechny úkoly v této kategorii
     ...
     if (_cachedTasks != null) {
       for (var i = 0; i < _cachedTasks!.length; i++) {
         if (_cachedTasks![i].category == categoryId) {
           _cachedTasks![i] = _cachedTasks![i].copyWith(category: defaultCategoryId);
         }
       }
     }
   });
 ...
 /// Označí úkol jako dokončený/nedokončený.
 ...
 /// Vymaže všechny úkoly a kategorie.
 Future<void> clearAllData() async {
   ...
   await _withRetry<void>(() async {
     ...
     // Přidáme smazání všech úkolů
     for (final doc in tasksSnapshot.docs) {
       batch.delete(doc.reference);
     }
     
     // Přidáme smazání všech kategorií
 ...
 Future<void> syncFromLocal(List<Task> localTasks, List<TaskCategory> localCategories) async {
   if (_userId == null) return;
   
   await _withRetry<void>(() async {
     // 1. Synchronizace kategorií
     final cloudCategoriesSnapshot = await _getCategoriesCollection().get();
     final cloudCategoryIds = cloudCategoriesSnapshot.docs.map((doc) => doc.id).toSet();
     ...
     // Odstranění kategorií, které nejsou v lokálních datech
     final categoriesToRemove = cloudCategoryIds.where((id) => !localCategoryIds.contains(id));
     ...
     // Odstranění úkolů, které nejsou v lokálních datech
     final tasksToRemove = cloudTaskIds.where((id) => !localTaskIds.contains(id));
 ...
 Future<DateTime?> getLastSyncTimestamp() async {
   try {
     ...
     if (doc.exists && doc.data() != null && doc.data()!['lastChecklistSync'] != null) {
