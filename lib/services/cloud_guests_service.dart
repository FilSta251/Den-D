...
class CloudGuestsService {
  ...
  Future<List<TableArrangement>> fetchTables() async {
    ...
    try {
      return await _withRetry<List<TableArrangement>>(() async {
        final snapshot = await _getTablesCollection()
            .orderBy('name')
            .get();
      ...
      if (_cachedGuests != null) {
        for (var i = 0; i < _cachedGuests!.length; i++) {
          if (_cachedGuests![i].table == tableName) {
            _cachedGuests![i] = _cachedGuests![i].copyWith(table: 'Nepřiřazen');
          }
        }
      }
    });
  ...
  Future<void> syncFromLocal(List<Guest> localGuests, List<TableArrangement> localTables) async {
    if (_userId == null) return;
    
    await _withRetry<void>(() async {
      // 1. Synchronizace stolů
      final cloudTablesSnapshot = await _getTablesCollection().get();
      final cloudTableIds = cloudTablesSnapshot.docs.map((doc) => doc.id).toSet();
      ...
      final tablesToRemove = cloudTableIds.where((id) => !localTableIds.contains(id));
      ...
      final cloudGuestIds = cloudGuestsSnapshot.docs.map((doc) => doc.id).toSet();
      ...
      final guestsToRemove = cloudGuestIds.where((id) => !localGuestIds.contains(id));
  ...
  Future<DateTime?> getLastSyncTimestamp() async {
    try {
      ...
      if (doc.exists && doc.data() != null && doc.data()!['lastGuestsSync'] != null) {
  ...
  Future<Map<String, dynamic>> getGuestStatistics() async {
    ...
    try {
      ...
      return {
        'total': guests.length,
        'male': guests.where((g) => g.gender == 'Muž').length,
        'female': guests.where((g) => g.gender == 'Žena').length,
        'other': guests.where((g) => g.gender == 'Jiné').length,
        'confirmed': guests.where((g) => g.attendance == 'Potvrzená').length,
        'declined': guests.where((g) => g.attendance == 'Neutvrzená').length,
        'pending': guests.where((g) => g.attendance == 'Neodpovězeno').length,
