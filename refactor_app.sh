...
# Nejprve vytvořme soubory pomocí tee (místo echo nebo here-dokumentů)
echo "===== Vytvářím AppRouter ====="
tee lib/router/app_router.dart > /dev/null << ENDFILE
...
tee lib/services/navigation_service.dart > /dev/null << ENDFILE
...
tee lib/theme/app_theme.dart > /dev/null << ENDFILE
...
tee lib/services/cloud_schedule_service.dart > /dev/null << ENDFILE
...
# Záloha původního service_locator.dart
cp lib/di/service_locator.dart service_locator.backup

# Příprava importů pro přidání do service_locator.dart
tee additional_imports.txt > /dev/null << ENDFILE
// Import pro ScheduleManager a NavigationService
...
# Vložení nových importů na začátek souboru
sed -i '1r additional_imports.txt' lib/di/service_locator.dart
rm additional_imports.txt

# Přidání registrací služeb
sed -i '/await locator.allReady();/i \
  // Registrace NavigationService \
  locator.registerLazySingleton<NavigationService>(() => NavigationService()); \
  \
  // Registrace CloudScheduleService \
  locator.registerLazySingleton<CloudScheduleService>(() => CloudScheduleService()); \
  \
  // Registrace LocalScheduleService \
...
echo "===== Vytváření ScheduleManager ====="
tee lib/services/schedule_manager.dart > /dev/null << ENDFILE
...
# Vytvoření upraveného main.dart
tee lib/main.dart > /dev/null << ENDFILE
import 'package:flutter/material.dart';
...
tee lib/providers/base_notifier.dart > /dev/null << ENDFILE
...
tee lib/widgets/global_widgets.dart > /dev/null << ENDFILE
...
# Vytvoření skriptu pro aktualizaci importů
tee update_imports.sh > /dev/null << ENDFILE
#!/bin/bash
