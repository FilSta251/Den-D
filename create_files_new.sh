...
def create_file(file_path, content):
    ...
    if os.path.exists(file_path):
        print(f"Soubor '{file_path}' již existuje, přeskakuji.")
    else:
        print(f"Vytvářím soubor: {file_path}")
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"Soubor '{file_path}' byl úspěšně vytvořen.")

# Vytvoření adresářové struktury
os.makedirs("test/unit", exist_ok=True)
os.makedirs("test/widget", exist_ok=True)
os.makedirs("lib/services", exist_ok=True)
os.makedirs("lib/utils", exist_ok=True)
os.makedirs("lib/widgets", exist_ok=True)

# Vytvoření souboru environment_config.dart
environment_config_content = '''// lib/services/environment_config.dart
...
# Vytvoření souboru connectivity_manager.dart
connectivity_manager_content = '''// lib/services/connectivity_manager.dart
...
# Vytvoření souborů
create_file("lib/services/environment_config.dart", environment_config_content)
create_file("lib/services/connectivity_manager.dart", connectivity_manager_content)
